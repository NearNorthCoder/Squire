...
/* loaded from: squire-homes-0.1.0.jar:gg/squire/mahoghomes/model/Home.class */
public enum Home {
    ...
    static {
        Home[] values;
        ImmutableSet.Builder<Integer> b = new ImmutableSet.Builder<>();
        for (Home h : values()) {
            b.add(h.getLadders());
        }
        LADDERS = b.build();
    ...
    Home(WorldArea area, String hint, int npcId, WorldPoint location, RequiredMaterialsByTier requiredMaterialsByTier, Integer... ladders) {
        this.area = area;
        this.hint = hint;
        this.npcId = npcId;
    ...
    public static boolean isLadder(int objID) {
        return LADDERS.contains(Integer.valueOf(objID));
    }
    ...
    public int[] getLadderIds() {
        return Stream.of((Object[]) this.ladders).mapToInt((v0) -> {
            return v0.intValue();
        }).toArray();
    }
}
