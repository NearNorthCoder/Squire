...
public class Perspective {
    ...
    public static Polygon getCanvasTilePoly(@Nonnull Client client, @Nonnull LocalPoint localLocation, int zOffset) {
        return Perspective.getCanvasTileAreaPoly(client, localLocation, 1, 1, -1, zOffset);
    }

    public static Polygon getCanvasTileAreaPoly(@Nonnull Client client, @Nonnull LocalPoint localLocation, int size) {
        return Perspective.getCanvasTileAreaPoly(client, localLocation, size, size, -1, 0);
    }
    ...
    public static Polygon getCanvasTileAreaPoly(@Nonnull Client client, @Nonnull LocalPoint localLocation, int sizeX, int sizeY, int plane, int zOffset) {
        ...
        int nwX = neX;
        int nwY = swY;
        int swHeight = Perspective.getHeight(scene, swX, swY, tilePlane) - zOffset;
        int nwHeight = Perspective.getHeight(scene, nwX, nwY, tilePlane) - zOffset;
        int neHeight = Perspective.getHeight(scene, neX, neY, tilePlane) - zOffset;
        int seHeight = Perspective.getHeight(scene, seX, seY, tilePlane) - zOffset;
    ...
    public static Shape getClickbox(@Nonnull Client client, Model model, int orientation, LocalPoint point) {
        ...
        int x = point.getX();
        int y = point.getY();
        int z = Perspective.getTileHeight(client, point, client.getPlane());
        return Perspective.getClickbox(client, model, orientation, x, y, z);
    }
    ...
    private static Polygon linePoly(@Nonnull Client client, @Nonnull LocalPoint startLocation, @Nonnull LocalPoint endLocation) {
        LocalPoint startPoint = new LocalPoint(startLocation.getX() - 64, startLocation.getY() + 64);
        LocalPoint endPoint = new LocalPoint(endLocation.getX() - 64, endLocation.getY() + 64);
    ...
    public static List<Polygon> getLinePolyList(@Nonnull Client client, @Nonnull WorldPoint startLocation, @Nonnull WorldPoint endLocation) {
        int j;
        Polygon p;
        LocalPoint localPointEnd;
        LocalPoint localPointStart;
        WorldPoint endPoint;
        ...
        for (i = 0; i < sizeX; ++i) {
            startPoint = new WorldPoint(startLocation.getX() + i, startLocation.getY() + sizeY - 1, startLocation.getPlane());
            endPoint = new WorldPoint(startLocation.getX() + (i + 1), startLocation.getY() + sizeY - 1, startLocation.getPlane());
            localPointStart = LocalPoint.fromWorld(client, startPoint);
            localPointEnd = LocalPoint.fromWorld(client, endPoint);
            if (localPointStart == null || localPointEnd == null || (p = Perspective.linePoly(client, localPointStart, localPointEnd)) == null) continue;
            pList.add(p);
        }
        for (i = 0; i < sizeX; ++i) {
            startPoint = new WorldPoint(startLocation.getX() + i, startLocation.getY() - 1, startLocation.getPlane());
            endPoint = new WorldPoint(startLocation.getX() + (i + 1), startLocation.getY() - 1, startLocation.getPlane());
            localPointStart = LocalPoint.fromWorld(client, startPoint);
            localPointEnd = LocalPoint.fromWorld(client, endPoint);
            if (localPointStart == null || localPointEnd == null || (p = Perspective.linePoly(client, localPointStart, localPointEnd)) == null) continue;
            pList.add(p);
        ...
        for (j = 0; j < sizeY; ++j) {
            startPoint = new WorldPoint(startLocation.getX(), startLocation.getY() + (j - 1), startLocation.getPlane());
            endPoint = new WorldPoint(startLocation.getX(), startLocation.getY() + j, startLocation.getPlane());
            localPointStart = LocalPoint.fromWorld(client, startPoint);
            localPointEnd = LocalPoint.fromWorld(client, endPoint);
            if (localPointStart == null || localPointEnd == null || (p = Perspective.linePoly(client, localPointStart, localPointEnd)) == null) continue;
            pList.add(p);
        ...
        for (j = 0; j < sizeY; ++j) {
            startPoint = new WorldPoint(startLocation.getX() + sizeX, startLocation.getY() + (j - 1), startLocation.getPlane());
            endPoint = new WorldPoint(startLocation.getX() + sizeX, startLocation.getY() + j, startLocation.getPlane());
            localPointStart = LocalPoint.fromWorld(client, startPoint);
            localPointEnd = LocalPoint.fromWorld(client, endPoint);
            if (localPointStart == null || localPointEnd == null || (p = Perspective.linePoly(client, localPointStart, localPointEnd)) == null) continue;
            pList.add(p);
