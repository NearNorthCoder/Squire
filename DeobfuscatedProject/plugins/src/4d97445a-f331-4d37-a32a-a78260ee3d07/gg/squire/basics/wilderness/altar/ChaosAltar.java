...
extends SquirePlugin {
    /* synthetic */ Map<KitType, ItemComposition> hh;
    private static /* synthetic */ int[] llIlIllII;
    /* synthetic */ boolean hi;

    protected Class<?>[] tasks() {
        Class[] classArray = new Class[llIlIllII[0]];
        classArray[ChaosAltar.llIlIllII[1]] = bX.class;
        return classArray;
    }

    static {
        ChaosAltar.lIlllIlllII();
    }
    ...
    private static void lIlllIlllII() {
        llIlIllII = new int[2];
        ChaosAltar.llIlIllII[0] = " ".length();
        ChaosAltar.llIlIllII[1] = (0x28 ^ 0x24 ^ (0x97 ^ 0x8D)) & (0x14 ^ 0x6F ^ (0 ^ 0x6D) ^ -" ".length());
    }

    private static boolean lIlllIllllI(int n2) {
        return n2 != 0;
    }
    ...
    public void b(PlayerSpawned playerSpawned) {
        void lIlIllllIllIlIl;
        if (ChaosAltar.lIlllIlllIl(playerSpawned.getPlayer(), Players.getLocal())) {
            return;
        }
        if (ChaosAltar.lIlllIllllI(PvPUtil.isAttackable((Client)Static.getClient(), (Player)lIlIllllIllIlIl.getPlayer()) ? 1 : 0)) {
            lIlIllllIllIllI.hi = llIlIllII[0];
            Game.logout();
        }
    }
    ...
    public void b(GameStateChanged gameStateChanged) {
        if (ChaosAltar.lIlllIlllIl(gameStateChanged.getGameState(), GameState.LOGIN_SCREEN)) {
            this.hi = llIlIllII[1];
        }
    }
