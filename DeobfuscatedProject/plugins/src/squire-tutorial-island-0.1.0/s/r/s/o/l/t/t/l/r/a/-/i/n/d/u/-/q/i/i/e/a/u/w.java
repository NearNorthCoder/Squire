/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  net.runelite.api.Locatable
 *  net.runelite.api.NPC
 *  net.runelite.api.coords.WorldPoint
 *  net.unethicalite.api.entities.NPCs
 *  net.unethicalite.api.entities.Players
 *  net.unethicalite.api.movement.Movement
 *  net.unethicalite.api.movement.Reachable
 *  net.unethicalite.api.movement.pathfinder.Walker
 */
package s.r.s.o.l.t.t.l.r.a.-.i.n.d.u.-.q.i.i.e.a.u;

import java.util.Random;
import net.runelite.api.Locatable;
import net.runelite.api.NPC;
import net.runelite.api.coords.WorldPoint;
import net.unethicalite.api.entities.NPCs;
import net.unethicalite.api.entities.Players;
import net.unethicalite.api.movement.Movement;
import net.unethicalite.api.movement.Reachable;
import net.unethicalite.api.movement.pathfinder.Walker;
import s.r.s.o.l.t.t.l.r.a.-.i.n.d.u.-.q.i.i.e.a.u.M;
import s.r.s.o.l.t.t.l.r.a.-.i.n.d.u.-.q.i.i.e.a.u.l;

public class w
implements l {
    private final /* synthetic */ String an;
    private final /* synthetic */ String ao;
    private static /* synthetic */ int ah;
    private static /* synthetic */ int[] lIlIIlIIlIIII;

    private static boolean lIlllIIlIIlIIlI(int n2, int n3) {
        return n2 != n3;
    }

    @Override
    public String z() {
        return this.ao;
    }

    private static boolean lIlllIIlIIlIIII(Object object) {
        return object == null;
    }

    public w(String string, String string2) {
        this.an = string;
        this.ao = string2;
    }

    static {
        w.lIlllIIlIIIllll();
    }

    private static boolean lIlllIIlIIlIIll(int n2) {
        return n2 == 0;
    }

    private static boolean lIlllIIlIIlIIIl(int n2, int n3) {
        return n2 >= n3;
    }

    /*
     * WARNING - void declaration
     */
    public void a(NPC nPC) {
        void var5_5;
        void var4_4;
        void var2_2;
        void llllllllllllllIlllIIIIIlllllllII;
        if (w.lIlllIIlIIlIIII(nPC)) {
            return;
        }
        WorldPoint llllllllllllllIlllIIIIIllllllIll = llllllllllllllIlllIIIIIlllllllII.getWorldLocation();
        Random llllllllllllllIlllIIIIIllllllIlI = new Random();
        int llllllllllllllIlllIIIIIllllllIIl = llllllllllllllIlllIIIIIllllllIlI.nextInt(lIlIIlIIlIIII[5]) - lIlIIlIIlIIII[3];
        int llllllllllllllIlllIIIIIllllllIII = llllllllllllllIlllIIIIIllllllIlI.nextInt(lIlIIlIIlIIII[5]) - lIlIIlIIlIIII[3];
        if (w.lIlllIIlIIlIIll(llllllllllllllIlllIIIIIllllllIIl) && w.lIlllIIlIIlIIll(llllllllllllllIlllIIIIIllllllIII)) {
            llllllllllllllIlllIIIIIllllllIIl = lIlIIlIIlIIII[0];
        }
        WorldPoint worldPoint = new WorldPoint(var2_2.getX() + var4_4, var2_2.getY() + var5_5, var2_2.getPlane());
        Walker.walkTo((WorldPoint)worldPoint);
        "".length();
        ah = M.a(lIlIIlIIlIIII[0], lIlIIlIIlIIII[4]);
    }

    /*
     * WARNING - void declaration
     */
    @Override
    public boolean D() {
        w llllllllllllllIlllIIIIlIIIIIIlII;
        void llllllllllllllIlllIIIIlIIIIIIIll;
        String[] stringArray = new String[lIlIIlIIlIIII[0]];
        stringArray[w.lIlIIlIIlIIII[1]] = this.an;
        NPC nPC = NPCs.getNearest((String[])stringArray);
        if (w.lIlllIIlIIlIIII(nPC)) {
            return lIlIIlIIlIIII[1];
        }
        if (w.lIlllIIlIIlIIIl(ah, lIlIIlIIlIIII[0])) {
            ah -= lIlIIlIIlIIII[0];
            return lIlIIlIIlIIII[1];
        }
        if (w.lIlllIIlIIlIIlI(Players.getLocal().getAnimation(), lIlIIlIIlIIII[2])) {
            return lIlIIlIIlIIII[1];
        }
        if (w.lIlllIIlIIlIIll(Movement.shouldWalk() ? 1 : 0)) {
            return lIlIIlIIlIIII[1];
        }
        if (w.lIlllIIlIIlIIll(Reachable.isInteractable((Locatable)llllllllllllllIlllIIIIlIIIIIIIll) ? 1 : 0)) {
            llllllllllllllIlllIIIIlIIIIIIlII.a((NPC)llllllllllllllIlllIIIIlIIIIIIIll);
            return lIlIIlIIlIIII[0];
        }
        if (w.lIlllIIlIIlIIIl(llllllllllllllIlllIIIIlIIIIIIIll.getWorldLocation().distanceTo((Locatable)Players.getLocal()), lIlIIlIIlIIII[3])) {
            llllllllllllllIlllIIIIlIIIIIIlII.a((NPC)llllllllllllllIlllIIIIlIIIIIIIll);
            return lIlIIlIIlIIII[0];
        }
        nPC.interact(this.ao);
        ah = M.a(lIlIIlIIlIIII[0], lIlIIlIIlIIII[4]);
        return lIlIIlIIlIIII[0];
    }

    @Override
    public String y() {
        return this.an;
    }

    private static void lIlllIIlIIIllll() {
        lIlIIlIIlIIII = new int[6];
        w.lIlIIlIIlIIII[0] = " ".length();
        w.lIlIIlIIlIIII[1] = (0x54 ^ 0x71) & ~(0x6E ^ 0x4B);
        w.lIlIIlIIlIIII[2] = -" ".length();
        w.lIlIIlIIlIIII[3] = "   ".length();
        w.lIlIIlIIlIIII[4] = 0x7F ^ 0x7B;
        w.lIlIIlIIlIIII[5] = 0x73 ^ 0x2B ^ (0x2D ^ 0x72);
    }
}

