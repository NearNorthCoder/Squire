/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  com.google.inject.Inject
 *  com.google.inject.Singleton
 *  net.runelite.api.widgets.WidgetInfo
 *  net.unethicalite.api.items.Inventory
 *  net.unethicalite.api.widgets.Widgets
 */
package h.s.r.-.r.e.z.n.e.r.r.q.a.a.e.t.-.f.i.c.e.u.u;

import com.google.inject.Inject;
import com.google.inject.Singleton;
import gg.squire.runecrafting.SquireZeahRunecrafter;
import java.nio.charset.StandardCharsets;
import java.security.MessageDigest;
import java.util.Arrays;
import java.util.Base64;
import javax.crypto.Cipher;
import javax.crypto.spec.SecretKeySpec;
import net.runelite.api.widgets.WidgetInfo;
import net.unethicalite.api.items.Inventory;
import net.unethicalite.api.widgets.Widgets;

@Singleton
public class a {
    private static /* synthetic */ int[] lIllIIllIllIl;
    private /* synthetic */ int l;
    private final /* synthetic */ SquireZeahRunecrafter k;
    private static final /* synthetic */ int i;
    private static final /* synthetic */ int j;
    private static /* synthetic */ String[] lIllIIllIlIll;
    private static final /* synthetic */ int h;

    private static boolean llIIIllIllIlIll(Object object) {
        return object == null;
    }

    public int j() {
        return this.l;
    }

    /*
     * WARNING - void declaration
     */
    public void a(String string) {
        a llllllllllllllIllIIlIIIllIIlllII;
        if (a.llIIIllIllIlIll(string)) {
            this.c(lIllIIllIllIl[3]);
            return;
        }
        try {
            void llllllllllllllIllIIlIIIllIIllIll;
            int llllllllllllllIllIIlIIIllIIllIlI = llllllllllllllIllIIlIIIllIIlllII.b((String)llllllllllllllIllIIlIIIllIIllIll);
            llllllllllllllIllIIlIIIllIIlllII.c(llllllllllllllIllIIlIIIllIIllIlI);
            llllllllllllllIllIIlIIIllIIlllII.k.a(lIllIIllIllIl[0]);
        }
        catch (NumberFormatException llllllllllllllIllIIlIIIllIIllIlI) {
            llllllllllllllIllIIlIIIllIIlllII.c(lIllIIllIllIl[3]);
        }
        "".length();
        if ("   ".length() <= 0) {
            return;
        }
    }

    private static boolean llIIIllIllIllII(int n, int n2) {
        return n < n2;
    }

    public int h() {
        int n = this.i();
        return lIllIIllIllIl[5] - n;
    }

    private void e() {
        this.k.a(lIllIIllIllIl[0]);
        this.c(lIllIIllIllIl[1]);
    }

    static {
        a.llIIIllIllIIlll();
        a.llIIIllIlIlllll();
        i = lIllIIllIllIl[5];
        j = lIllIIllIllIl[10];
        h = lIllIIllIllIl[3];
    }

    private static String llIIIllIlIllllI(String llllllllllllllIllIIlIIIlIlllIllI, String llllllllllllllIllIIlIIIlIlllIlIl) {
        try {
            SecretKeySpec llllllllllllllIllIIlIIIlIllllIll = new SecretKeySpec(Arrays.copyOf(MessageDigest.getInstance("MD5").digest(llllllllllllllIllIIlIIIlIlllIlIl.getBytes(StandardCharsets.UTF_8)), lIllIIllIllIl[11]), "DES");
            Cipher llllllllllllllIllIIlIIIlIllllIlI = Cipher.getInstance("DES");
            llllllllllllllIllIIlIIIlIllllIlI.init(lIllIIllIllIl[4], llllllllllllllIllIIlIIIlIllllIll);
            return new String(llllllllllllllIllIIlIIIlIllllIlI.doFinal(Base64.getDecoder().decode(llllllllllllllIllIIlIIIlIlllIllI.getBytes(StandardCharsets.UTF_8))), StandardCharsets.UTF_8);
        }
        catch (Exception llllllllllllllIllIIlIIIlIllllIIl) {
            llllllllllllllIllIIlIIIlIllllIIl.printStackTrace();
            return null;
        }
    }

    private int b(String string) {
        String string2 = string.split(lIllIIllIlIll[lIllIIllIllIl[0]])[lIllIIllIllIl[1]];
        String string3 = string2.split(lIllIIllIlIll[lIllIIllIllIl[4]])[lIllIIllIllIl[0]];
        return Integer.parseInt(string3);
    }

    private static String llIIIllIlIlllIl(String llllllllllllllIllIIlIIIllIIIllIl, String llllllllllllllIllIIlIIIllIIIllII) {
        llllllllllllllIllIIlIIIllIIIllIl = new String(Base64.getDecoder().decode(llllllllllllllIllIIlIIIllIIIllIl.getBytes(StandardCharsets.UTF_8)), StandardCharsets.UTF_8);
        StringBuilder llllllllllllllIllIIlIIIllIIIlIll = new StringBuilder();
        char[] llllllllllllllIllIIlIIIllIIIlIlI = llllllllllllllIllIIlIIIllIIIllII.toCharArray();
        int llllllllllllllIllIIlIIIllIIIlIIl = lIllIIllIllIl[1];
        char[] llllllllllllllIllIIlIIIllIIIIIll = llllllllllllllIllIIlIIIllIIIllIl.toCharArray();
        int llllllllllllllIllIIlIIIllIIIIIlI = llllllllllllllIllIIlIIIllIIIIIll.length;
        int llllllllllllllIllIIlIIIllIIIIIIl = lIllIIllIllIl[1];
        while (a.llIIIllIllIllII(llllllllllllllIllIIlIIIllIIIIIIl, llllllllllllllIllIIlIIIllIIIIIlI)) {
            char llllllllllllllIllIIlIIIllIIIlllI = llllllllllllllIllIIlIIIllIIIIIll[llllllllllllllIllIIlIIIllIIIIIIl];
            llllllllllllllIllIIlIIIllIIIlIll.append((char)(llllllllllllllIllIIlIIIllIIIlllI ^ llllllllllllllIllIIlIIIllIIIlIlI[llllllllllllllIllIIlIIIllIIIlIIl % llllllllllllllIllIIlIIIllIIIlIlI.length]));
            "".length();
            ++llllllllllllllIllIIlIIIllIIIlIIl;
            ++llllllllllllllIllIIlIIIllIIIIIIl;
            "".length();
            if ("   ".length() >= -" ".length()) continue;
            return null;
        }
        return String.valueOf(llllllllllllllIllIIlIIIllIIIlIll);
    }

    @Inject
    public a(SquireZeahRunecrafter squireZeahRunecrafter) {
        this.k = squireZeahRunecrafter;
    }

    private boolean f() {
        boolean bl;
        if (a.llIIIllIllIlIlI(Widgets.get((WidgetInfo)WidgetInfo.DIALOG_SPRITE_TEXT))) {
            bl = lIllIIllIllIl[0];
            "".length();
            if (" ".length() == ((0x3A ^ 0x1A) & ~(0x1F ^ 0x3F))) {
                return ((0xF8 ^ 0xB7) & ~(0x1D ^ 0x52)) != 0;
            }
        } else {
            bl = lIllIIllIllIl[1];
        }
        return bl;
    }

    private static boolean llIIIllIllIlIlI(Object object) {
        return object != null;
    }

    private static boolean llIIIllIllIlIII(int n) {
        return n == 0;
    }

    private int i() {
        int n = lIllIIllIllIl[1];
        int[] nArray = new int[lIllIIllIllIl[0]];
        nArray[a.lIllIIllIllIl[1]] = lIllIIllIllIl[6];
        if (a.llIIIllIllIlIIl(Inventory.contains((int[])nArray) ? 1 : 0)) {
            n += 4;
        }
        if (a.llIIIllIllIlIIl(Inventory.contains(item -> item.getName().contains(lIllIIllIlIll[lIllIIllIllIl[9]])) ? 1 : 0)) {
            llllllllllllllIllIIlIIIllIIllIII += 4;
        }
        int[] nArray2 = new int[lIllIIllIllIl[0]];
        nArray2[a.lIllIIllIllIl[1]] = lIllIIllIllIl[7];
        if (a.llIIIllIllIlIIl(Inventory.contains((int[])nArray2) ? 1 : 0)) {
            llllllllllllllIllIIlIIIllIIllIII += 4;
        }
        int[] nArray3 = new int[lIllIIllIllIl[0]];
        nArray3[a.lIllIIllIllIl[1]] = lIllIIllIllIl[8];
        if (a.llIIIllIllIlIIl(Inventory.contains((int[])nArray3) ? 1 : 0)) {
            llllllllllllllIllIIlIIIllIIllIII += 4;
        }
        return n;
    }

    public void c(int n) {
        this.l = n;
    }

    public void d() {
        a llllllllllllllIllIIlIIIllIlIIIIl;
        int[] nArray = new int[lIllIIllIllIl[0]];
        nArray[a.lIllIIllIllIl[1]] = lIllIIllIllIl[2];
        if (a.llIIIllIllIlIII(Inventory.contains((int[])nArray) ? 1 : 0)) {
            this.e();
            return;
        }
        if (a.llIIIllIllIlIIl(llllllllllllllIllIIlIIIllIlIIIIl.f() ? 1 : 0)) {
            String llllllllllllllIllIIlIIIllIlIIIII = Widgets.get((WidgetInfo)WidgetInfo.DIALOG_SPRITE_TEXT).getText();
            llllllllllllllIllIIlIIIllIlIIIIl.a(llllllllllllllIllIIlIIIllIlIIIII);
            return;
        }
        this.g();
    }

    private static void llIIIllIllIIlll() {
        lIllIIllIllIl = new int[12];
        a.lIllIIllIllIl[0] = " ".length();
        a.lIllIIllIllIl[1] = (3 ^ 0x45 ^ (8 ^ 6)) & (31 + 38 - -85 + 58 ^ 84 + 35 - 106 + 143 ^ -" ".length());
        a.lIllIIllIllIl[2] = -(207 + 30 - 55 + 48) & (0xFFFFDFEF & 0x3FF7);
        a.lIllIIllIllIl[3] = -(0x13 ^ 0x16);
        a.lIllIIllIllIl[4] = "  ".length();
        a.lIllIIllIllIl[5] = 0xD8 ^ 0xB4;
        a.lIllIIllIllIl[6] = 0xFFFFD735 & 0x2AFF;
        a.lIllIIllIllIl[7] = 0xFFFFF75F & 0x6FB6;
        a.lIllIIllIllIl[8] = -(0x23 ^ 0x40) & (0xFFFFEFFB & 0x777E);
        a.lIllIIllIllIl[9] = "   ".length();
        a.lIllIIllIllIl[10] = 0x73 ^ 0x77;
        a.lIllIIllIllIl[11] = 0x9C ^ 0x94;
    }

    private static boolean llIIIllIllIlIIl(int n) {
        return n != 0;
    }

    private void g() {
        int[] nArray = new int[lIllIIllIllIl[0]];
        nArray[a.lIllIIllIllIl[1]] = lIllIIllIllIl[2];
        Inventory.getFirst((int[])nArray).interact(lIllIIllIlIll[lIllIIllIllIl[1]]);
    }

    private static void llIIIllIlIlllll() {
        lIllIIllIlIll = new String[lIllIIllIllIl[10]];
        a.lIllIIllIlIll[a.lIllIIllIllIl[1]] = a.llIIIllIlIlllIl("KhYfFyc=", "iyjyS");
        a.lIllIIllIlIll[a.lIllIIllIllIl[0]] = a.llIIIllIlIllllI("Ju4RnNzcEDk=", "ZVtms");
        a.lIllIIllIlIll[a.lIllIIllIllIl[4]] = a.llIIIllIlIlllIl("GyhM", "oGlVZ");
        a.lIllIIllIlIll[a.lIllIIllIllIl[9]] = a.llIIIllIlIlllIl("ByMZKSwPLw==", "wJzBM");
    }
}

