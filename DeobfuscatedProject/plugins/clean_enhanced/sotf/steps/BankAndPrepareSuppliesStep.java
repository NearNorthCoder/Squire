...
public class BankAndPrepareSuppliesStep implements QuestStep {
    ...
    // State management
    public static List<ShopItem> shoppingList;                        // List of items to buy
    ...
    static {
        shoppingList = new ArrayList<ShopItem>();
    ...
    private static void buildShoppingList() {
        ...
        if (!Bank.contains(crossbowCheck)) {
            ShopItem crossbowItem = new ShopItem(CROSSBOW_ID, 2, GameStateUtil.randomRange(7932, 6258));
        ...
        if (!Bank.contains(silverBarCheck)) {
            ShopItem silverBarItem = new ShopItem(221, 22508, 20);
        ...
        if (!Bank.contains((item) -> item.getName().toLowerCase().contains(ITEM_RING_OF_WEALTH))) {
            ShopItem ringItem = new ShopItem(8151, 5, 28664);
        ...
        if (!Bank.contains(teleportCheck)) {
            ShopItem teleportItem = new ShopItem(16342, 1, 31159);
        ...
        if (!Bank.contains(varrockTeleportCheck)) {
            ShopItem varrockItem = new ShopItem(VARROCK_TELEPORT_ID, 4, Constants.DEFAULT_GE_TIMEOUT);
        ...
        if (!Bank.contains(houseTabCheck)) {
            ShopItem houseTabItem = new ShopItem(8013, 20, 7933);
        ...
        if (!Bank.contains(staminaCheck)) {
            ShopItem staminaItem = new ShopItem(STAMINA_POTION_ID, 32, 8111);
