...
/**
 * NMZ (Nightmare Zone) Melee quest step.
 * This step ensures specific quests are completed before enabling NMZ melee mode.
 *
 * Quest progression:
 ...
 *
 * Once all quests are complete, enables NMZ melee mode.
 */
public class NmzMeleeQuestStep implements QuestStep {
    ...
    private static final String COMBAT_STYLE = "melee";

    // Quest name
    private static final String QUEST_NAME = "NMZ Melee ";
    ...
    public static void updateQuestProgress() {
        ...
        // All quests complete - enable NMZ melee mode
        if (GameStateUtil.getVarbit(VAMPIRE_SLAYER_VARBIT) >= VAMPIRE_SLAYER_COMPLETE_VALUE
                && GameStateUtil.getVarbit(LOST_CITY_VARBIT) >= LOST_CITY_COMPLETE_VALUE
                && GameStateUtil.getVarbit(FIGHT_ARENA_VARBIT) >= FIGHT_ARENA_COMPLETE_VALUE
                && GameStateUtil.getVarbit(TREE_GNOME_VILLAGE_VARBIT) >= TREE_GNOME_VILLAGE_COMPLETE_VALUE
