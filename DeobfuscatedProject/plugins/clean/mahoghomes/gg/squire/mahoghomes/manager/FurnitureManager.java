...
public class FurnitureManager {
    public static final Predicate<String> FURNITURE_ACTION_PREDICATE = action -> "Build".equals(action) || "Remove".equals(action) || "Repair".equals(action);
    ...
    public TileObject getCurrentFurniture() {
        ...
        if (this.currentFurniture == null || this.currentFurniture.getPlane() != this.client.getPlane() || !Hotspot.requiresAttention(this.currentFurniture.getId()) || !current.getArea().contains2D(this.currentFurniture.getWorldLocation())) {
            ...
            this.currentFurniture = TileObjects.getAll(to -> Hotspot.requiresAttention(to.getId()) && current.getArea().contains2D(to.getWorldLocation())).stream().min(Comparator.comparingInt(to -> to.distanceTo(compare))).orElse(null);
        }
        if (this.currentFurniture != null && (this.currentFurniture.getActions() == null || TileObjects.getNearest((int[])new int[]{this.currentFurniture.getId()}) == null)) {
