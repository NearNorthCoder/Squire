/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  net.unethicalite.api.items.Equipment
 *  net.unethicalite.api.items.Inventory
 */
package e.q.u.d.w.s.p.e.e.c.s.h.i.-.l.r.-.r.l.a.o.h.u.l.e;

import e.q.u.d.w.s.p.e.e.c.s.h.i.-.l.r.-.r.l.a.o.h.u.l.e.ZEnum;
import java.nio.charset.StandardCharsets;
import java.security.MessageDigest;
import java.util.Arrays;
import java.util.Base64;
import javax.crypto.Cipher;
import javax.crypto.spec.SecretKeySpec;
import net.unethicalite.api.items.Equipment;
import net.unethicalite.api.items.Inventory;

public class a_Unknown {
    private static /* synthetic */ String[] lllIIIlIIIIl;
    private static /* synthetic */ int[] lllIIIlIIIlI;

    private static boolean lIIIlIIIlllIlIl(int n2) {
        return n2 != 0;
    }

    static {
        a.lIIIlIIIlllIIll();
        a.lIIIlIIIlllIIlI();
    }

    private static String lIIIlIIIlllIIIl(String var14, String var17) {
        var14 = new String(Base64.getDecoder().decode(var14.getBytes(StandardCharsets.UTF_8)), StandardCharsets.UTF_8);
        StringBuilder var10 = new StringBuilder();
        char[] var18 = var17.toCharArray();
        int var8 = lllIIIlIIIlI[1];
        char[] var16 = var14.toCharArray();
        int var1 = var16.length;
        int var2 = lllIIIlIIIlI[1];
        while (a.lIIIlIIIllllIIl(var2, var1)) {
            char var12 = var16[var2];
            var10.append((char)(var12 ^ var18[var8 % var18.length]));
            0;
            ++var8;
            ++var2;
            0;
            if (1 == 1) continue;
            return null;
        }
        return String.valueOf(var10);
    }

    private static boolean lIIIlIIIllllIIl(int n2, int n3) {
        return n2 < n3;
    }

    private static boolean lIIIlIIIlllIlll(int n2, int n3) {
        return n2 >= n3;
    }

    private static boolean lIIIlIIIlllIlII(int n2) {
        return n2 == 0;
    }

    public static boolean n() {
        boolean bl;
        if (a.lIIIlIIIlllIllI(Inventory.getFirst(item -> {
            int n2;
            if (a.lIIIlIIIlllIlIl(item.getName().contains(lllIIIlIIIIl[lllIIIlIIIlI[9]]) ? 1 : 0) && a.lIIIlIIIlllIlll(item.getQuantity(), lllIIIlIIIlI[5])) {
                n2 = lllIIIlIIIlI[2];
                0;
                if (((0x71 ^ 0x23) & ~(0x54 ^ 6)) <= -1) {
                    return false;
                }
            } else {
                n2 = lllIIIlIIIlI[1];
            }
            return n2 != 0;
        }))) {
            bl = lllIIIlIIIlI[2];
            0;
            if (((32 + 124 - 59 + 128 ^ 119 + 39 - -6 + 30) & (123 + 86 - 187 + 126 ^ 108 + 165 - 91 + 1 ^ -1)) != ((189 + 20 - 47 + 83 ^ 62 + 88 - 80 + 80) & (247 + 180 - 250 + 72 ^ 79 + 117 - 53 + 11 ^ -1))) {
                return ((0x7F ^ 0xC ^ (0xF ^ 0x6B)) & (0xD3 ^ 0x80 ^ (0xD0 ^ 0x94) ^ -1)) != 0;
            }
        } else {
            bl = lllIIIlIIIlI[1];
        }
        return bl;
    }

    /*
     * Enabled force condition propagation
     * Lifted jumps to return sites
     */
    public static boolean k() {
        boolean bl;
        block3: {
            block2: {
                String[] stringArray = new String[lllIIIlIIIlI[0]];
                stringArray[a.lllIIIlIIIlI[1]] = lllIIIlIIIIl[lllIIIlIIIlI[1]];
                stringArray[a.lllIIIlIIIlI[2]] = lllIIIlIIIIl[lllIIIlIIIlI[2]];
                if (!a.lIIIlIIIlllIlII(Inventory.contains((String[])stringArray) ? 1 : 0)) break block2;
                String[] stringArray2 = new String[lllIIIlIIIlI[2]];
                stringArray2[a.lllIIIlIIIlI[1]] = lllIIIlIIIIl[lllIIIlIIIlI[0]];
                if (!a.lIIIlIIIlllIlIl(Equipment.contains((String[])stringArray2) ? 1 : 0)) break block3;
            }
            bl = lllIIIlIIIlI[2];
            0;
            if (3 != ((0x2B ^ 0x2D ^ (0x2E ^ 0x36)) & (0xB8 ^ 0xAF ^ (0xAC ^ 0xA5) ^ -1))) return bl;
            return ((0x6D ^ 0x3A ^ (0x25 ^ 0x62)) & (0x37 ^ 0x23 ^ (0x2F ^ 0x2B) ^ -1)) != 0;
        }
        bl = lllIIIlIIIlI[1];
        return bl;
    }

    public static boolean l() {
        String[] stringArray = new String[lllIIIlIIIlI[2]];
        stringArray[a.lllIIIlIIIlI[1]] = lllIIIlIIIIl[lllIIIlIIIlI[3]];
        return Inventory.contains((String[])stringArray);
    }

    public static boolean o() {
        boolean bl;
        String[] stringArray = new String[lllIIIlIIIlI[2]];
        stringArray[a.lllIIIlIIIlI[1]] = lllIIIlIIIIl[lllIIIlIIIlI[4]];
        if (a.lIIIlIIIlllIlIl(Equipment.contains((String[])stringArray) ? 1 : 0)) {
            String[] stringArray2 = new String[lllIIIlIIIlI[2]];
            stringArray2[a.lllIIIlIIIlI[1]] = lllIIIlIIIIl[lllIIIlIIIlI[5]];
            if (a.lIIIlIIIlllIlll(Inventory.getCount((String[])stringArray2), lllIIIlIIIlI[2])) {
                bl = lllIIIlIIIlI[2];
                0;
                
                }
            } else {
                bl = lllIIIlIIIlI[1];
                0;
                if (-1 >= 1) {
                    return ((0 ^ 0x47 ^ (0x45 ^ 0x12)) & (0x13 ^ 0x77 ^ (0x50 ^ 0x24) ^ -1)) != 0;
                }
            }
        } else {
            String[] stringArray3 = new String[lllIIIlIIIlI[2]];
            stringArray3[a.lllIIIlIIIlI[1]] = lllIIIlIIIIl[lllIIIlIIIlI[6]];
            if (a.lIIIlIIIlllIlll(Inventory.getCount((String[])stringArray3), lllIIIlIIIlI[0])) {
                bl = lllIIIlIIIlI[2];
                0;
                if (((85 + 100 - 85 + 49 ^ 80 + 50 - 28 + 74) & (0x42 ^ 0x2B ^ (0xC0 ^ 0x8C) ^ -1)) != ((109 + 112 - 180 + 104 ^ 43 + 32 - -15 + 60) & (0x15 ^ 0x7E ^ (0x33 ^ 0x5F) ^ -1))) {
                    return ((130 + 149 - 102 + 5 ^ 1 + 48 - -15 + 85) & (0xBB ^ 0x8A ^ (0x74 ^ 0x66) ^ -1)) != 0;
                }
            } else {
                bl = lllIIIlIIIlI[1];
            }
        }
        return bl;
    }

    private static boolean lIIIlIIIlllIllI(Object object) {
        return object != null;
    }

    private static void lIIIlIIIlllIIll() {
        lllIIIlIIIlI = new int[12];
        a.lllIIIlIIIlI[0] = 2;
        a.lllIIIlIIIlI[1] = (0x96 ^ 0xC0 ^ (0x56 ^ 5)) & (0xE ^ 0x72 ^ (0x30 ^ 0x49) ^ -1);
        a.lllIIIlIIIlI[2] = 1;
        a.lllIIIlIIIlI[3] = 3;
        a.lllIIIlIIIlI[4] = 0x65 ^ 0 ^ (0xC7 ^ 0xA6);
        a.lllIIIlIIIlI[5] = 0x13 ^ 0x2D ^ (0x18 ^ 0x23);
        a.lllIIIlIIIlI[6] = 0x78 ^ 0x1B ^ (0xF7 ^ 0x92);
        a.lllIIIlIIIlI[7] = 0xFFFFB5FA & 0x6ECF;
        a.lllIIIlIIIlI[8] = -(0xFFFFDDA3 & 0x2E5F) & (0xFFFFFD97 & 0x6EFB);
        a.lllIIIlIIIlI[9] = 0xB9 ^ 0x99 ^ (0xAA ^ 0x8D);
        a.lllIIIlIIIlI[10] = 0x76 ^ 0x7E;
        a.lllIIIlIIIlI[11] = 0x5B ^ 0x1E ^ (0x71 ^ 0x3D);
    }

    private static void lIIIlIIIlllIIlI() {
        lllIIIlIIIIl = new String[lllIIIlIIIlI[11]];
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[1]] = a."Hammer";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[2]] = a."Hallowed hammer";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[0]] = a."Hallowed hammer";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[3]] = a."Saw";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[4]] = a."Hallowed symbol";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[5]] = a."Vampyre dust";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[6]] = a."Vampyre dust";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[9]] = a." nails";
        a.lllIIIlIIIIl[a.lllIIIlIIIlI[10]] = a."plank";
    }

    private static String lIIIlIIIllIllll(String var5, String var9) {
        try {
            SecretKeySpec var11 = new SecretKeySpec(Arrays.copyOf(MessageDigest.getInstance("MD5").digest(var9.getBytes(StandardCharsets.UTF_8)), lllIIIlIIIlI[10]), "DES");
            Cipher var3 = Cipher.getInstance("DES");
            var3.init(lllIIIlIIIlI[0], var11);
            return new String(var3.doFinal(Base64.getDecoder().decode(var5.getBytes(StandardCharsets.UTF_8))), StandardCharsets.UTF_8);
        }
        catch (Exception var19) {
            var19.printStackTrace();
            return null;
        }
    }

    public static boolean p() {
        int[] nArray = new int[lllIIIlIIIlI[0]];
        nArray[a.lllIIIlIIIlI[1]] = lllIIIlIIIlI[7];
        nArray[a.lllIIIlIIIlI[2]] = lllIIIlIIIlI[8];
        return Equipment.contains((int[])nArray);
    }

    public static boolean m() {
        boolean bl;
        if (a.lIIIlIIIlllIllI(Inventory.getFirst(item -> {
            int n2;
            if (a.lIIIlIIIlllIlIl(item.getName().toLowerCase().contains(lllIIIlIIIIl[lllIIIlIIIlI[10]]) ? 1 : 0)) {
                int[] nArray = new int[lllIIIlIIIlI[2]];
                nArray[a.lllIIIlIIIlI[1]] = item.getId();
                if (a.lIIIlIIIlllIlll(Inventory.getCount((int[])nArray), lllIIIlIIIlI[0])) {
                    n2 = lllIIIlIIIlI[2];
                    0;
                    if (((0xA3 ^ 0xAB ^ 1) & (41 + 114 - 86 + 105 ^ 97 + 70 - 131 + 131 ^ -1)) == 0) return n2 != 0;
                    return ((0x30 ^ 0x66 ^ (0x3F ^ 0x35)) & (136 + 156 - 245 + 172 ^ 90 + 4 - 91 + 132 ^ -1)) != 0;
                }
            }
            n2 = lllIIIlIIIlI[1];
            return n2 != 0;
        }))) {
            bl = lllIIIlIIIlI[2];
            0;
            if ((171 + 166 - 213 + 52 ^ 148 + 111 - 226 + 147) != (0x68 ^ 0x42 ^ (0x31 ^ 0x1F))) {
                return false;
            }
        } else {
            bl = lllIIIlIIIlI[1];
        }
        return bl;
    }

    private static String lIIIlIIIlllIIII(String var13, String var6) {
        try {
            SecretKeySpec var4 = new SecretKeySpec(MessageDigest.getInstance("MD5").digest(var6.getBytes(StandardCharsets.UTF_8)), "Blowfish");
            Cipher var7 = Cipher.getInstance("Blowfish");
            var7.init(lllIIIlIIIlI[0], var4);
            return new String(var7.doFinal(Base64.getDecoder().decode(var13.getBytes(StandardCharsets.UTF_8))), StandardCharsets.UTF_8);
        }
        catch (Exception var15) {
            var15.printStackTrace();
            return null;
        }
    }

    public static boolean a(z z2) {
        if (a.lIIIlIIIllllIII((Object)z2, (Object)z.NONE)) {
            return lllIIIlIIIlI[1];
        }
        return z2.spell.canCast();
    }

    private static boolean lIIIlIIIllllIII(Object object, Object object2) {
        return object == object2;
    }
}

