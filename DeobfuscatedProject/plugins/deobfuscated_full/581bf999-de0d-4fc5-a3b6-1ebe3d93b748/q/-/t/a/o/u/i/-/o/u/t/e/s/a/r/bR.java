...
/* TASK: Dodging poison splat -> DodgingpoisonsplatTask */
...
extends bX {
    ...
    public boolean bL() {
        ...
        if (bR.lIlIlIIIIIllIl(var3.stream().noneMatch(arg_0 -> bR.c((Player)var1, arg_0)) ? 1 : 0)) {
            return llIlIllllII[0];
        }
        Log.info((String)llIlIlllIll[llIlIllllII[0]]);
        WorldArea var5 = var1.getWorldArea().offset(llIlIllllII[1]);
        WorldPoint var4 = var5.toWorldPointList().stream().filter(arg_0 -> bR.e((List)var3, arg_0)).filter(Reachable::isWalkable).filter(worldPoint -> {
            boolean bl2;
            ...
            return bl2;
        }).min(Comparator.comparingInt(arg_0 -> bR.d((Player)var1, arg_0))).orElse(null);
        if (bR.lIlIlIIIIIlllI(var4)) {
            return llIlIllllII[0];
        }
    ...
    private static String lIlIlIIIIIlIlI(String var9, String var6) {
        try {
            SecretKeySpec var7 = new SecretKeySpec(Arrays.copyOf(MessageDigest.getInstance("MD5").digest(var6.getBytes(StandardCharsets.UTF_8)), llIlIllllII[4]), "DES");
            Cipher var8 = Cipher.getInstance("DES");
            var8.init(llIlIllllII[5], var7);
