...
public class ItemPackets {
    ...
    public static void queueSwapItems(Item first, Item second) {
        Client client = Static.getClient();
        Widget clickedWidget = client.getWidget(0x950000).getChild(first.getSlot());
        Widget draggedOnWidget = client.getWidget(0x950000).getChild(second.getSlot());
        if (clickedWidget.getOnDragCompleteListener() != null) {
            ScriptEvent scriptEvent = client.createScriptEvent(clickedWidget.getOnDragCompleteListener());
            scriptEvent.setSource(clickedWidget);
            scriptEvent.setDragTarget(draggedOnWidget);
            scriptEvent.setMouseX(draggedOnWidget.getOriginalX());
            scriptEvent.setMouseY(draggedOnWidget.getOriginalY());
            GameThread.invoke(() -> client.runScriptEvent(scriptEvent));
        }
        ClientPacket clientPacket = Game.getClientPacket();
        PacketBufferNode packetBufferNode = Static.getClient().preparePacket(clientPacket.OPHELDD(), client.getPacketWriter().getIsaacCipher());
        packetBufferNode.getPacketBuffer().writeIntLE(draggedOnWidget.getId());
        packetBufferNode.getPacketBuffer().writeShortAddLE(clickedWidget.getItemId());
        packetBufferNode.getPacketBuffer().writeShort(draggedOnWidget.getIndex());
        packetBufferNode.getPacketBuffer().writeInt(clickedWidget.getId());
        packetBufferNode.getPacketBuffer().writeShortAddLE(draggedOnWidget.getItemId());
        packetBufferNode.getPacketBuffer().writeShortAddLE(clickedWidget.getIndex());
        GameThread.invoke(() -> ((PacketBufferNode)packetBufferNode).send());
    }
}
