...
extends Items {
    ...
    private Equipment() {
        super(InventoryID.EQUIPMENT, (Item item) -> {
            WidgetInfo widgetInfo = Equipment.getEquipmentWidgetInfo(item.getSlot());
            if (widgetInfo != null) {
    ...
    public static ItemQuery query() {
        return Equipment.query(Equipment::getAll);
    }
    ...
    public static List<Item> getAll() {
        return Equipment.getAll((Item x) -> true);
    }
    ...
    private static WidgetInfo getEquipmentWidgetInfo(int itemIndex) {
        for (EquipmentInventorySlot equipmentInventorySlot : EquipmentInventorySlot.values()) {
            if (equipmentInventorySlot.getSlotIdx() != itemIndex) continue;
    ...
    public static boolean containsAllOf(int ... ids) {
        for (int id : ids) {
            if (id == -1 || id == 0 || EQUIPMENT.exists(id)) continue;
            return false;
        }
        return true;
    }

    public static boolean containsAllOf(List<Integer> equipmentIdList) {
        for (int id : equipmentIdList) {
            if (id == -1 || id == 0 || EQUIPMENT.exists(id)) continue;
            return false;
        }
        return true;
    ...
    public static Item fromSlot(EquipmentInventorySlot slot) {
        return Equipment.getFirst((Item x) -> slot.getWidgetInfo().getPackedId() == x.getWidgetId());
    }
}
