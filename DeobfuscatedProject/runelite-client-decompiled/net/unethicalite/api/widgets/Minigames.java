...
public class Minigames {
    ...
    public static void teleport(Destination destination) {
        ...
        List<Integer> teleportGraphics = List.of(Integer.valueOf(800), Integer.valueOf(802), Integer.valueOf(803), Integer.valueOf(804));
        if (Minigames.isOpen() && minigamesTeleportButton != null) {
            if (Destination.getCurrent() != destination) {
                GameThread.invoke(() -> Static.getClient().runScript(new Object[]{124, destination.index}));
                return;
            }
            ...
            if (Widgets.isVisible(button)) {
                button.interact(Predicates.textContains((String)"Teleport to"));
            }
        } else {
            Minigames.open();
        }
    ...
    public static boolean open() {
        Widget widget;
        if (!Minigames.isTabOpen()) {
            Tabs.open(Tab.CLAN_CHAT);
            return false;
        }
        if (!Minigames.isOpen() && Widgets.isVisible(widget = MINIGAMES_TAB_BUTTON.get())) {
    ...
    public static Instant getLastMinigameTeleportUsage() {
        return Instant.ofEpochSecond((long)Vars.getVarp(888) * 60L);
    }
    ...
    public static enum Destination {
        ...
        public static Destination getCurrent() {
            ...
            if (Widgets.isVisible(selectedTeleport)) {
                return Destination.byName(selectedTeleport.getText());
            }
            return NONE;
        }

        public static Destination byName(String name) {
            return Arrays.stream(Destination.values()).filter(x -> x.getName().equals(name)).findFirst().orElse(NONE);
        }
        ...
        private Destination(int index, String name, WorldPoint location, boolean members) {
            this.index = index;
            this.name = name;
