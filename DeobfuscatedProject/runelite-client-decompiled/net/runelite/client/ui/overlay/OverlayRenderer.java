...
extends MouseAdapter {
    ...
    private void safeRender(Overlay overlay, Graphics2D graphics, Point point) {
        ...
        Dimension dimension = MoreObjects.firstNonNull(overlayDimension, new Dimension());
    ...
    private Point clampOverlayLocation(int overlayX, int overlayY, int overlayWidth, int overlayHeight, Overlay overlay) {
        ...
        return new Point(Ints.constrainToRange(overlayX, parentBounds.x, Math.max(parentBounds.x, parentBounds.x + parentBounds.width - overlayWidth)), Ints.constrainToRange(overlayY, parentBounds.y, Math.max(parentBounds.y, parentBounds.y + parentBounds.height - overlayHeight)));
    }
}
