...
extends JDialog {
    ...
    public FatalErrorDialog(String message) {
        ...
        this.setTitle("Fatal error starting Squire");
        ...
        this.title = new JLabel("There was a fatal error starting Squire");
    ...
    public static void showNetErrorWindow(String action, Throwable err) {
        if (err instanceof VerificationException || err instanceof GeneralSecurityException) {
            new FatalErrorDialog("Squire was unable to verify the security of its connection to the internet while " + action + ". You may have a misbehaving antivirus, internet service provider, a proxy, or an incomplete java installation.").addHelpButtons().open();
            return;
        }
        if (err instanceof ConnectException) {
            new FatalErrorDialog("Squire is unable to connect to a required server while " + action + ". Please check your internet connection").addHelpButtons().open();
            return;
        }
        if (err instanceof UnknownHostException) {
            new FatalErrorDialog("Squire is unable to resolve the address of a required server while " + action + ". Your DNS resolver may be misconfigured, pointing to an inaccurate resolver, or your internet connection may be down. ").addHelpButtons().addButton("Change your DNS resolver", () -> LinkBrowser.browse(RuneLiteProperties.getDNSChangeLink())).open();
            return;
        }
        new FatalErrorDialog("Squire encountered a fatal error while " + action + ".").addHelpButtons().open();
    }
}
