...
extends PluginPanel {
    ...
    private static final ImmutableList<String> CATEGORY_TAGS = ImmutableList.of("OpenOSRS", "Combat", "Chat", "Item", "Minigame", "Notification", "Plugin Hub", "Skilling", "XP");
    ...
    @Inject
    public PluginListPanel(ConfigManager configManager, PluginManager pluginManager, ExternalPluginManager externalPluginManager, OPRSExternalPluginManager oprsExternalPluginManager, final EventBus eventBus, Provider<ConfigPanel> configPanelProvider) {
        super(false);
    ...
    private void onSearchBarChanged() {
        ...
        List<PluginListItem> search = PluginSearch.search(this.pluginList, text);
        Comparator<PluginListItem> nameComparator = Comparator.comparing(PluginListItem::getPluginConfig, Comparator.comparing(PluginConfigurationDescriptor::getName));
        Comparator<PluginListItem> comparator = Comparator.comparing(PluginListItem::isPinned).reversed().thenComparing(nameComparator);
        search.sort(comparator);
        search.forEach(this.mainPanel::add);
        this.revalidate();
    }
