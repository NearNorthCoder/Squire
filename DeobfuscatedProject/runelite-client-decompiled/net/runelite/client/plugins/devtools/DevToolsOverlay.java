...
extends Overlay {
    ...
    private void renderTileTooltip(Graphics2D graphics, Tile tile) {
        ...
        if (poly != null && poly.contains(this.client.getMouseCanvasPosition().getX(), this.client.getMouseCanvasPosition().getY())) {
            WorldPoint worldLocation = WorldPoint.fromLocalInstance((Client)this.client, (LocalPoint)tileLocalLocation);
            WorldPoint instancedWorldLocation = WorldPoint.fromLocal((Client)this.client, (LocalPoint)tileLocalLocation);
            byte flags = this.client.getTileSettings()[tile.getRenderLevel()][tile.getSceneLocation().getX()][tile.getSceneLocation().getY()];
            Object tooltip = String.format("World location: %d, %d, %d</br>Flags: %d", worldLocation.getX(), worldLocation.getY(), worldLocation.getPlane(), flags);
            if (!instancedWorldLocation.equals((Object)worldLocation)) {
                tooltip = (String)tooltip + String.format("</br>Instance location: %d, %d, %d", instancedWorldLocation.getX(), instancedWorldLocation.getY(), instancedWorldLocation.getPlane());
            }
    ...
    private void renderMovementInfo(Graphics2D graphics, Tile tile) {
        ...
        if (this.client.getCollisionMaps() != null) {
            ...
            this.toolTipManager.add(new Tooltip("Reachable: " + Reachable.isWalkable(tile.getWorldLocation())));
            this.toolTipManager.add(new Tooltip("Interactable: " + Reachable.isInteractable((Locatable)tile)));
            OverlayUtil.renderPolygon(graphics, poly, GREEN);
        }
    }
