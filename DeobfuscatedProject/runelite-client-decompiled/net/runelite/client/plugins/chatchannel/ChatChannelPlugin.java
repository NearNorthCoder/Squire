...
extends Plugin {
    ...
    private void addActivityMessage(FriendsChatMember member, ActivityType activityType) {
        ...
        if (this.client.isResized() && this.client.getVarbitValue(4608) == 1) {
            textColor = MoreObjects.firstNonNull(this.chatColorConfig.transparentFriendsChatInfo(), JagexColors.CHAT_FC_TEXT_TRANSPARENT_BACKGROUND);
            channelColor = MoreObjects.firstNonNull(this.chatColorConfig.transparentFriendsChatChannelName(), JagexColors.CHAT_FC_NAME_TRANSPARENT_BACKGROUND);
        } else {
            textColor = MoreObjects.firstNonNull(this.chatColorConfig.opaqueFriendsChatInfo(), JagexColors.CHAT_FC_TEXT_OPAQUE_BACKGROUND);
            channelColor = MoreObjects.firstNonNull(this.chatColorConfig.opaqueFriendsChatChannelName(), JagexColors.CHAT_FC_NAME_OPAQUE_BACKGROUND);
        }
    ...
    private void addClanActivityMessage(ClanChannelMember member, ActivityType activityType, MemberActivity.ChatType chatType) {
        ...
        Color textColor = this.client.isResized() && this.client.getVarbitValue(4608) == 1 ? MoreObjects.firstNonNull(chatType == MemberActivity.ChatType.CLAN_CHAT ? this.chatColorConfig.transparentClanChatInfo() : this.chatColorConfig.transparentClanChatGuestInfo(), JagexColors.CHAT_FC_TEXT_TRANSPARENT_BACKGROUND) : MoreObjects.firstNonNull(chatType == MemberActivity.ChatType.CLAN_CHAT ? this.chatColorConfig.opaqueClanChatInfo() : this.chatColorConfig.opaqueClanChatGuestInfo(), JagexColors.CHAT_FC_TEXT_OPAQUE_BACKGROUND);
    ...
    public void onScriptCallbackEvent(ScriptCallbackEvent scriptCallbackEvent) {
        switch (scriptCallbackEvent.getEventName()) {
            case "confirmFriendsChatKick": {
                ...
                String kickPlayerName = (String)stringStack[stringSize - 1];
            ...
            case "chatMessageBuilding": {
                ...
                if ((rank = this.getRank(Text.removeTags(name = (String)(stringStack = this.client.getObjectStack())[(stringSize = this.client.getStringStackSize()) - 3]))) == null || rank == FriendsChatRank.UNRANKED || (iconNumber = this.chatIconManager.getIconNumber(rank)) <= -1) break;
