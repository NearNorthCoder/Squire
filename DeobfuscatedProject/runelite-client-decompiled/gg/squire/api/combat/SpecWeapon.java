...
public enum SpecWeapon {
    ...
    public Item getInventoryItem() {
        int map = ItemVariationMapping.map(this.getItemID());
        return Inventory.getFirst(i -> ItemVariationMapping.getVariations(map).contains(i.getId()));
    }
    ...
    public boolean matches(Item item) {
        ...
        int map = ItemVariationMapping.map(item.getId());
        return ItemVariationMapping.getVariations(map).contains(this.getItemID());
    }
    ...
    public static SpecWeapon forId(int itemId) {
        ...
        for (SpecWeapon specWeapon : SpecWeapon.values()) {
            if (!ItemVariationMapping.getVariations(map).contains(specWeapon.getItemID())) continue;
            return specWeapon;
        }
        return null;
    }
