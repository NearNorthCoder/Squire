...
extends Plugin {
    ...
    public void onMouseRecordingChange(RecordingChangeRequested recording) {
        if (recording.isCancelled()) {
            ...
            if (old != null && !this.getClass().isAnnotationPresent(SquireUtil.class)) {
                MouseRecording.deprecate(old);
            }
    ...
    protected boolean setMouseRecordings() {
        ...
        if (bestFitting.size() == 0) {
            ...
            SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(ClientUI.getFrame(), "No mouse recordings have been made!\nPlease make some mouse recordings before starting this plugin.\n", "Error starting plugin", 2));
            return false;
        }
    ...
    public static boolean setGlobalMouseRecordings() {
        ...
        int[] regions = Static.getClient().getMapRegions();
        List<MouseRecording> bestFitting = MouseRecording.findBestFitting(regions);
        if (bestFitting.size() == 0) {
            return true;
        }
        ThreadLocalRandom random = ThreadLocalRandom.current();
    ...
    public String getResourceName() {
        PluginDescriptor pds = this.getClass().getAnnotation(PluginDescriptor.class);
        if (pds == null) {
            return "unknown";
        }
    ...
    protected void startUp() throws Exception {
        ...
        if (conf) {
            ...
            if (!this.setMouseRecordings()) {
                ...
                log.info("Stopping plugin {} because mouse recordings could not be set.", (Object)this.getClass().getSimpleName());
                this.forceStop();
                return;
            }
            ...
            this.forceStop();
            log.info("Stopping plugin {} because authentication failed (could be instance limit too).", (Object)this.getClass().getSimpleName());
        }
    }
        ...
        Executors.newScheduledThreadPool(1).schedule(() -> {
            try {
                ...
                SwingUtilities.invokeLater(() -> {
                    try {
                        Static.getPluginManager().stopPlugin(this);
                    }
            ...
            Static.getPluginManager().setPluginEnabled(this, false);
            this.eventBus.post(new ExternalPluginsChanged());
        }, 1L, TimeUnit.SECONDS);
    }

    public boolean isUtility() {
        return this.getClass().isAnnotationPresent(SquireUtil.class);
    }
