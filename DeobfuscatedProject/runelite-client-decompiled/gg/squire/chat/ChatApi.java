...
public class ChatApi {
    ...
    public static CustomResponseMessage getCustomResponseMessage(ChatMessage message, List<ChatMessage> history) {
        ...
        try {
            CustomResponseMessage responseMessage = GSON.fromJson(messageJson, CustomResponseMessage.class);
    ...
    private static String getMessage(MessageWithContext messageWithContext) {
        CompletionRequest completionRequest = CompletionRequest.builder().model(Model.GPT4TURBO.getApiName()).numberOfChoices(1).messages(List.of(CompletionRequest.Message.builder().role("system").content(String.format(SYSTEM_MESSAGE, Players.getLocal().getName())).build(), CompletionRequest.Message.builder().content(GSON.toJson(messageWithContext)).role("user").build())).build();
