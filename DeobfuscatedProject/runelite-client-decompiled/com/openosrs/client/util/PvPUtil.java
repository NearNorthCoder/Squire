 ...
 *  net.runelite.api.geometry.Cuboid
 */
...
public class PvPUtil {
    ...
    public static int getWildernessLevelFrom(WorldPoint point) {
        ...
        if (MAIN_WILDERNESS_CUBOID.contains(point)) {
            ...
            return (point.getY() - 3520) / 8 + 1;
        }
        if (GOD_WARS_WILDERNESS_CUBOID.contains(point)) {
            return (point.getY() - 9920) / 8 - 1;
        }
        if (WILDERNESS_UNDERGROUND_CUBOID.contains(point)) {
            return (point.getY() - 9920) / 8 + 1;
        }
        return 0;
    }
    ...
    public static boolean isAttackable(Client client, Player player) {
        ...
        if (WorldType.isDeadmanWorld((Collection)client.getWorldType())) {
            return true;
        }
        if (WorldType.isPvpWorld((Collection)client.getWorldType())) {
            wildernessLevel += 15;
        }
        if (client.getVarbitValue(5963) == 1) {
            wildernessLevel += PvPUtil.getWildernessLevelFrom(client.getLocalPlayer().getWorldLocation());
        }
    ...
    public static int calculateRisk(Client client, ItemManager itemManager) {
        ...
        Item[] items = ArrayUtils.addAll(Objects.requireNonNull(client.getItemContainer(InventoryID.EQUIPMENT)).getItems(), Objects.requireNonNull(client.getItemContainer(InventoryID.INVENTORY)).getItems());
        TreeMap<Integer, Item> priceMap = new TreeMap<Integer, Item>(Comparator.comparingInt(Integer::intValue));
        int wealth = 0;
        for (Item i : items) {
