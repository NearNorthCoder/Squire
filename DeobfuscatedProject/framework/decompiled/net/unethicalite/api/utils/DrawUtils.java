...
public class DrawUtils {
    ...
    public static void drawTransports(Graphics2D graphics2D) {
        ...
        for (Transport transport : transports) {
            Point linkCenter;
            OverlayUtil.fillTile((Graphics2D)graphics2D, (Client)client, (WorldPoint)transport.getSource(), (Color)TRANSPORT_COLOR);
            Point center = Perspective.tileCenter((Client)client, (WorldPoint)transport.getSource());
            if (center == null || (linkCenter = Perspective.tileCenter((Client)client, (WorldPoint)transport.getDestination())) == null) continue;
    ...
    public static void drawCollisions(Graphics2D graphics2D, CollisionMap collisionMap) {
        ...
        for (Tile tile : tiles) {
            String s;
            Polygon poly = Perspective.getCanvasTilePoly((Client)client, (LocalPoint)tile.getLocalLocation());
    ...
    public static void drawCollisions(Graphics2D graphics2D) {
        DrawUtils.drawCollisions(graphics2D, Static.getGlobalWithLocalCollisions());
    }
}
